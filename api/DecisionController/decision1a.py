    ...
    def execute(self):
        ...
        select_stmt = "SELECT store_key, sales " \
              "FROM (" \
               ...
               ") subquery " \
               "WHERE rank <= 10 " \
               "ORDER BY rank".format(self.year, self.month, self.days_start, self.days_end)
        cur.execute(select_stmt)
        records = cur.fetchall()

        df = pd.DataFrame(records, columns=["Item Name", "Sales"])
        return df.to_dict(orient='records')


if __name__ == '__main__':
    decision1a = Decision1a(year=2018, month=5, days_start=1, days_end=30)
    data = decision1a.execute()
    print(data)
